...
def global_setting(request):
    active_categories = m_models.Category.objects.filter(active=True).order_by('id')
    return {
...
class Home(View):
    def get(self, request):
        ...
        all_articles_exists = all_articles.exists()
        # 获取所有推荐的文章
        top_articles = m_models.Article.objects.all().filter(is_recommend=1)
        top_articles_exists = top_articles.exists()
        # 首页分页功能
        try:
        ...
        return render(request, 'home.html', {
            ...
            'all_articles_exists': all_articles_exists,
            'top_articles': top_articles,
            'top_articles_exists': top_articles_exists
        })

# 分类清单
class CategoryList(View):
    def get(self, request):
        categories = m_models.Category.objects.all()
